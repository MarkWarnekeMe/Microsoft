...
az group create -g "$resource_group_name" -l "$resource_location" -o json
az storage account create -n "$resource_name" -g "$resource_group_name" -l "$resource_location" --sku Standard_LRS -o json
az storage container create -n "$container_name" --account-name "$resource_name" -o json

# Create a KeyVault
az keyvault create --location "$resource_location" --name "$resource_name" -g "$resource_group_name" -o json
